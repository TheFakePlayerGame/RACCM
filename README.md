...
# Download
To install this, obtain a release package from the distributor and extract the files into a folder on your desktop and run RBX Alt Manager.exe.
...
**A:** Open source programs such as this program are commonly detected as viruses because actual malware may use similar libraries. The Account Control feature uses websockets to connect to clients. If you'd like, you can download Visual Studio and compile this program on your own.
...
**A:** Use RAMDecrypt (not linked) to decrypt saved account files if needed. Ensure files being decrypted originate from your PC.
...
| Account Control | Control your in-game accounts using the `Account Control` window in RAM | Click `Account Control` in the main window - Documentation is included in the Nexus/NexusDocs.md file |
