...
namespace RBX_Alt_Manager.Forms
{
    public partial class AccountControl : Form
    {
        ...
        private void NexusDocsButton_Click(object sender, EventArgs e)
        {
            try
            {
                string localPath = Path.Combine(Application.StartupPath, "Nexus", "NexusDocs.md");

                if (File.Exists(localPath))
                    Process.Start(new ProcessStartInfo(localPath) { UseShellExecute = true });
                else
                    MessageBox.Show("Nexus documentation not found locally.", "Nexus Docs", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch { }
        }
