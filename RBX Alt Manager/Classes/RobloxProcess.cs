...
namespace RBX_Alt_Manager.Classes
{
    internal class RobloxProcess
    {
        ...
        public static async void UpdateMatches()
        {
            try
            {
                // Load regex matches from local Resources/WatcherRegexMatches.txt instead of fetching from GitHub
                string localPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Resources", "WatcherRegexMatches.txt");

                if (File.Exists(localPath))
                {
                    string MatchList = File.ReadAllText(localPath);
                ...
                {
                    Program.Logger.Warn($"WatcherRegexMatches.txt not found at {localPath}; using built-in defaults.");
                }
            }
